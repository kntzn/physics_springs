...
Во-вторых, запись уравнений в таком порядке, как в примере с кодом выше (такая запись, кстати, называется явным интегрированием по Эйлеру), приведет к погрешности, поскольку обновление координат происходит до обновления скорости. Чтобы внести ясность, представим результат единичного выполнения этих двх строк:
      
      r = 0, V = 0, F = 1, m = 1;
      r += V*dt;
      V += (F/m)*dt;
      
Первая строка не увеличит <b>r</b>, поскольку <b>V</b> равно нулю. А вот скорость, в свою очередь, возрастет на <b>dt</b>: 
